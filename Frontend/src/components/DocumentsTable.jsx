...
const DocumentsTable = ({ documents, onView, onDownload, onDelete, onUpload, onEdit }) => {
    ...
    const getStatusBadge = (status) => {
        ...
        if (status === 'Secondary Completion') {
            return <span className="px-3 py-1 rounded-full text-xs font-bold bg-indigo-50 text-indigo-700 border border-indigo-100 whitespace-nowrap">Secondary</span>;
        }
    ...
    return (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex-1">
                            ...
                            <tr key={doc.id} className="hover:bg-gray-50/50 transition-colors group">
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex items-center justify-end gap-1">
                                        <button title="View Document" onClick={() => onView(doc)} className="p-2 rounded-lg text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all cursor-pointer"><Eye size={16} /></button>
            ...
            <div className="md:hidden p-4 space-y-4 bg-gray-50/30">
                {documents.map((doc) => (
                    <div key={doc.id} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm hover:border-indigo-200 transition-all relative">
                        ...
                        <div className="flex items-center justify-end gap-1 border-t border-gray-50 pt-3 mt-2">
                            <button onClick={() => onView(doc)} className="flex items-center justify-center gap-1.5 flex-1 py-1.5 rounded-lg text-sm font-bold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 transition-all cursor-pointer"><Eye size={15} /> View</button>
